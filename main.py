...
def add_employee(employee_id: str, name: str, department: str, work_lat: float, work_lng: float, db: Session = Depends(get_db)):
    ...
    # حفظ الموظف مع موقع عمله المخصص له
    ...
    return {"status": "success", "message": f"تم تسجيل الموظف {name} بموقعه المخصص بنجاح!"}
...
def record_attendance(employee_id: str, action_type: str, lat: float, lng: float, db: Session = Depends(get_db)):
    ...
    # 2. توسيع النطاق المسموح به إلى 200 متر
    ALLOWED_RADIUS = 200 
    
    # 3. حساب المسافة
    distance = calculate_distance(lat, lng, employee.work_lat, employee.work_lng)
    
    # 4. الحظر إذا تجاوز المسافة الجديدة
    if distance > ALLOWED_RADIUS:
        raise HTTPException(
            status_code=400, 
            detail=f"فشل الحضور: أنت خارج النطاق المسموح به. المسافة الحالية: {round(distance)} متر، والنطاق المتاح هو {ALLOWED_RADIUS} متر فقط."
        )
    
    # 5. حساب التأخير في حال الدخول
    delay = 0
    if action_type == "دخول":
    ...
    msg = f"تم تسجيل {action_type} بنجاح في موقعك المخصص!"
    if delay > 0:
        msg += f" (تنبيـه: لديك تأخير {delay} دقيقة)"
